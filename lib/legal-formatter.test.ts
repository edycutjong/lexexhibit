...
describe('legal-formatter', () => {
  ...
  describe('formatParty', () => {
    it('should format defendant wallet correctly', () => {
      expect(formatParty('0x1234567890abcdef1234567890abcdef12345678', true))
        .toBe('the Defendant wallet ending in 5678');
    });

    it('should format non-defendant wallet correctly', () => {
      expect(formatParty('0x1234567890abcdef1234567890abcdef12345678', false))
        .toBe('the wallet ending in 5678');
    });
  });
  ...
  describe('formatValue', () => {
    ...
    it('should default to "tokens" as symbol', () => {
      expect(formatValue('500')).toBe('500 tokens');
    });
  });
  ...
  describe('formatLegalProse', () => {
    ...
    it('should default to "engaged in an asset movement" for unknown categories', () => {
      const prose = formatLegalProse(timestamp, from, to, value, symbol, 'unknown', 'A-5');
      expect(prose).toContain('engaged in an asset movement');
    });
  });
});
